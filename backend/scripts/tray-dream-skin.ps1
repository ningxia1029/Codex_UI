...
try {
  ...
  function Add-DreamSkinTrayItem {
    param(
      # ToolStripItemCollection 是可枚举集合。PowerShell 在集合为空时会将
      # 强类型参数展开为“空集合”，从而导致参数绑定失败；保留为对象后再调用
      # .Add()，可以稳定处理主菜单和子菜单的空集合。
  ...
  function Rebuild-DreamSkinTrayMenu {
    ...
    if ($savedThemes.Count -eq 0) {
      ...
      foreach ($saved in $savedThemes) {
        ...
        # 避免 PowerShell 对 DropDownItems 集合的参数绑定异常。
        $savedItem = [System.Windows.Forms.ToolStripMenuItem]::new($savedName)
        $savedItem.add_Click({
          try { & $savedAction } catch { Show-DreamSkinTrayError -Message $_.Exception.Message }
        }.GetNewClosure())
        [void]$savedMenu.DropDownItems.Add($savedItem)
      }
    }
