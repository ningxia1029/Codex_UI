...
function Get-DreamSkinNodeRuntime {
  ...
  # GUI 发布包通过环境变量提供已验证的私有 Node；独立脚本仍可回退系统 PATH。
  $candidate = "$env:CODEX_DREAM_SKIN_NODE".Trim()
  $nodePath = $null
  if ($candidate) {
    $candidateName = [System.IO.Path]::GetFileName($candidate)
    if ((Test-Path -LiteralPath $candidate -PathType Leaf) -and $candidateName -ieq 'node.exe') {
      $nodePath = (Resolve-Path -LiteralPath $candidate -ErrorAction Stop).Path
    }
  }
  ...
  if (-not $nodePath) {
    ...
    if (-not $command) { throw "Node.js $MinimumMajor or newer is required and was not found in PATH or the packaged runtime." }
  ...
  # 使用调用方明确指定或从 PATH 解析到的可执行文件本身。process.execPath
  # 在少数 Windows 安装器环境会回报原安装目录，不能据此覆盖包内路径。
  return [pscustomobject]@{ Path = $nodePath; Version = $version; Major = $major }
}
