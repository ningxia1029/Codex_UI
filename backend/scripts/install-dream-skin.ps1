...
try {
  ...
  try {
    ...
    # 现有配置含 [desktop.*] 子表时，上游安全检查会拒绝重写。
    # 注入主题仍可按当前原生外观运行，因此保留用户配置并继续安装。
    if ($_.Exception.Message -like '*nested desktop tables*') {
      Write-Warning 'Skipped base appearance configuration because config.toml contains nested [desktop.*] tables. Existing Codex settings were left unchanged.'
    } else {
      throw
    }
  }
